...
inline void __initialize_data(uint32_t* flash_begin, uint32_t* data_begin, uint32_t* data_end){
  uint32_t* dataInit = data_begin;
  while (dataInit < data_end)
  {
    *dataInit++ = *flash_begin++; 
  }
}

inline void __initialize_bss(uint32_t* bss_begin, uint32_t* bss_end){
  uint32_t* dataInit = bss_begin;
  while (dataInit < bss_end)
  {
    *dataInit++ = 0;
  }
...
__WEAK void	DMA2_Channel6_IRQHandler(void)        { Default_Handler(); }
__WEAK void	DMA2_Channel7_IRQHandler(void)        { Default_Handler(); }
